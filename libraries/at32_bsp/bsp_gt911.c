...
error_status touch_read_xy(uint16_t *x, uint16_t *y)
{
  ...
  error_status status = ERROR;
    ...
    gt9111_reg_read(GT_STS_REG, buf, 6);
     /* clear touch flag */
    ...
    {       
        switch(touch_dev_struct.direction){
          ...
          case TOUCH_SCAN_HORIZONTAL: 
            *y = 320 - (((uint16_t)buf[3] << 8) + buf[2]);
            *x = (((uint16_t)buf[5] << 8) + buf[4]);   
          break;
          case TOUCH_SCAN_VERTICAL_180:
            *x = 480- (((uint16_t)buf[3] << 8) + buf[2]);
            *y = 320- (((uint16_t)buf[5] << 8) + buf[4]);              
            break;
          case TOUCH_SCAN_HORIZONTAL_180:
            *y = (((uint16_t)buf[3] << 8) + buf[2]);
            *x = 480-(((uint16_t)buf[5] << 8) + buf[4]);               
            break;
        }
				status = SUCCESS;
    }  
...
#define false 0
#define true 1
error_status touch_read(uint16_t *x, uint16_t *y)
{
  ...
  switch(touch_state){
    ...
    case 2:
      if(touch_read_xy(&xp, &yp)){ 
				count = 0;				
        if((abs(last_x-xp)>50)||(abs(last_y-yp)>50)){
           touch_state = 0;
        }
      }else{
				count++;
