...
void lcd_fill_circle(uint16_t x,uint16_t y,uint16_t r,uint16_t color)
{
	...
	for(i=0;i<=r;i++)     //从圆心到边缘循环，对称地画两条线，画之前要找到合适的长度
	{
		...
		lcd_draw_line(i+x,y-temp,i+x,y+temp,color);
		lcd_draw_line(x-i,y-temp,x-i,y+temp,color);
	}
} 
...
{
  const unsigned int Chinese32[] = {'车', '車', '马', '象', '相', '士', '将', '帅', '炮', '卒', '兵'};
  u16 x, y, GBK, camp;
  u16 ex, ey;
  u8 index1, i, j;
...
void chess_drawline(u16 x1, u16 y1, u16 x2, u16 y2,u16 color)
{
	u16 t; 
	int xerr=0,yerr=0,delta_x,delta_y,distance; 
	int incx,incy,uRow,uCol; 
	...
	{  
		lcd_point_draw(uRow,uCol,color);//画点 
		xerr+=delta_x ; 
		yerr+=delta_y ; 
		if(xerr>distance) 
		{ 
...
{
	const unsigned int Chinese64[] = {'无','效','将','军','红','黑','方','获','胜'};
	int8_t index1=0,i;
	u16 j;
	u8 byteColNum; //一列的字节数
	u16 byteNum;
   ...
   byteColNum = (length/8.0) + ((length%8)?1:0);
	...
	for(j=0;j<=byteNum;j++)
	{
		for(i=7;i>=0;i--)       
		{
			...
			lcd_point_draw(x,y,color);
			else
			lcd_point_draw(x,y,BROWN);
			y++;
		}
